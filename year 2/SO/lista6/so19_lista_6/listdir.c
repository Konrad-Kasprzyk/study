...
static void print_mode(mode_t m) {
  ...
  // set-uid -> plik wykonuje się z uprawnieniami użytkownika, który utworzył plk, mimo że wykonuje go użytkownik z innymi uprawnieniami
  // set-gid -> to samo ale na grupach
  ...
  // If setuid/setgid/sticky bit is set but there is no execute permission then it's uppercase
...
static void file_info(int dirfd, const char *name) {
  ...
  // 4.24 w APUE wyjaśnia character special file i block special file. 
