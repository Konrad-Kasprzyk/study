...
static arena_t *init_arena(arena_t *ar) {
  ...
  // arena_t ma tablicę bitów rozmiaru zero, więc osobno trzeba znaleźć rozmiar
  size_t free_space = (ARENA_SIZE - sizeof(arena_t));
  // zwraca rozmiar tablicy, argumant to ile ma bitów w sobie ( typu bitstr_t )
...
static void *alloc_block(arena_t *ar) {
  /* assert wypisuje błąd gdy warunek nie jest spełniony */
  assert(ar->nfree > 0);
  int index;
  /* TODO: Calculate index of free block and mark it used, update nfree. */

  // ustawia index na pozycje gdzie znalazł pusty bit, -1 gdy nie znalazł.  Iteruje się po wszystkich bitach w bitmapie
...
static void free_block(arena_t *ar, void *ptr) {
  ...
  // Czy blok mieści się w rozmiarze listy
  assert(index < ar->nitems);
  // Czy blok nie został wcześniej usunięty
