...
// ustawia w arenie, że wszystkie bloki są wolne, tworzy je
// zapełnia arenę po kolei największymi blokami jakie można dać
...
// data = początek areny, cur_offset = gdzie w liście jesteśmy i +1, bo jeden bajt na metadane
// początek danych w bloku po metadanych ( 1 bajt )
#define CURRENT_PTR(data) (data + cur_offset + 1)


// data oznacza arenę, przechowujemy w niej napisy, czyli tablice int8_t.   len to długość tablicy do zaalokowania
// curr_offset wyznacza nam gdzie w liście danej areny jesteśmy, np. po metadanych na pierwszym bloku listy
static void *alloc_block(int8_t *data, uint8_t len) {
  ...
  while (!END_OF_CHUNK(data)) {
    if (BLOCK_USED(data)) {
      ...
      // alokujemy tyle miejsca ile potrzebujemy, resztę oznaczamy jako wolny blok o odpowiednim rozmiarze
      ...
      // jeśli nie istnieje to wyjdziemy z while, bo END_OD_CHUNK wykryje koniec list, czyli zero w nagłówku bloku
      MOVE_NEXT(data);
    } else if (NEXT_BLOCK_SIZE(data) <= len - BLOCK_SIZE(data)) {
      // w tym bloku nie ma miejsca na cały napis, następny blok jest wolny i za mały
      ...
      // powiększamy nasz blok, tyle ile potrzebujemay, by pomieścić napis długości len. Następny blok odpowiednio zmniejszamy
  ...
  // zwracamy wskaźnik gdzie zaalokowaliśmy pamięć. Testy w test.h tego potrzebują
  return result;
}
...
// str to wskażnik na początek danych w bloku, czyli po metadanych bloku ( 1 bajt )
static void strfree(char *str) {
  ...
  // ptr to teraz początek bloku, wsakzuje na metadane bloku, blok zajęty -> ujemna wartość pamięci
  assert(*ptr > 0);
  *ptr *= -1;
}

// sprawdza, czy wszystko jest dobrze na listach
